...
if manu == 'AL':
     ...
     if vn == 'Fuel Injector':
         fig = px.scatter(al, x=al['master_row_id'], y=al['error_iq'], color='master_row_id')
         st.plotly_chart(fig)
         ...
         fig = px.scatter(al, x=al['master_row_id'], y=al['error_rp'], color='master_row_id')
         st.plotly_chart(fig)
         st.image('Common Rail.png', width=150, caption='Injector')
         col1, col2 = st.columns([0.25, 0.25])
         with col1:
             al_avg1 = np.mean(al['error_iq'])
             val = float('{:.2f}'.format(al_avg1))
        ...
        fig = px.scatter(al, x=al['master_row_id'], y=al['error_mu'], color='master_row_id')
        st.plotly_chart(fig)
        st.image('Metering Unit.png', width=150, caption='Injector')
        col1, col2 = st.columns([0.25, 0.25])
        with col1:
            al_avg1 = np.mean(al['error_iq'])
            val = float('{:.2f}'.format(al_avg1))
...
if manu == 'TATA':
     ...
     st.write('## DASHBOARD REQUESTED')
     if vn == 'Fuel Injector':
         fig = px.scatter(al, x=tata['master_row_id'], y=tata['error_iq'], color='master_row_id')
         st.plotly_chart(fig)
         st.image('Injector.png', width=150, caption='Injector')
         col1, col2 = st.columns([0.25, 0.25])
         with col1:
        ...
        fig = px.scatter(al, x=tata['master_row_id'], y=tata['error_rp'], color='master_row_id')
        st.plotly_chart(fig)
        st.image('Common Rail.png', width=150, caption='Injector')
        col1, col2 = st.columns([0.25, 0.25])
        with col1:
        ...
        fig = px.scatter(al, x=tata['master_row_id'], y=tata['error_mu'], color='master_row_id')
        st.plotly_chart(fig)
        st.image('Metering Unit.png', width=150, caption='Injector')
        col1, col2 = st.columns([0.25, 0.25])
        with col1:
