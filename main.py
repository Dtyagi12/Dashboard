...
if manu == 'AL':
     ...
     if vn == 'Fuel Injector':
         fig = sns.scatterplot(x=al['master_row_id'], y=al['error_iq'])
         st.plotly_chart(fig)
         ...
         fig = sns.scatterplot(x=al['master_row_id'], y=al['error_rp'])
         st.plotly_chart(fig)
        ...
        fig = sns.scatterplot(x=al['master_row_id'], y=al['error_mu'])
        st.plotly_chart(fig)
...
if manu == 'TATA':
     ...
     st.write('## DASHBOARD REQUESTED')
     if vn == 'Fuel Injector':
         fig = sns.scatter(x=tata['master_row_id'], y=tata['error_iq'])
         st.plotly_chart(fig)
         st.image('Injector.png', width=150, caption='Injector')
         col1, col2 = st.columns([0.25, 0.25])
         with col1:
        ...
        fig = sns.scatterplot(x=tata['master_row_id'], y=tata['error_rp'])
        st.plotly_chart(fig)
        st.image('Common Rail.png')
        col1, col2 = st.columns([0.25, 0.25])
        with col1:
        ...
        fig = sns.scatterplot(x=tata['master_row_id'], y=tata['error_mu'])
        st.plotly_chart(fig)
        st.image('Metering Unit.png')
        col1, col2 = st.columns([0.25, 0.25])
        with col1:
